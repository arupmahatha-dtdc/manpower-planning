...
def main() -> None:
    ...
    with cols_in[0]:
        input_per_day_ndx = st.number_input('Per Day NDX', value=default_ndx, step=100, format='%d')
    with cols_in[1]:
        input_per_day_dox = st.number_input('Per Day DOX', value=default_dox, step=100,format='%d')
